...
export async function PATCH(
  ...
  try {
    ...
    const id = path.split('/')[3]; // Format: /api/announcements/[id]/publish
    ...
    if (user?.role !== 'ADMIN' && user?.role !== 'MODERATOR') {
      return NextResponse.json(
        { error: 'Vous n\'avez pas les permissions nécessaires pour publier une annonce' },
        { status: 403 }
      );
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'Une erreur est survenue lors de la publication de l\'annonce';
